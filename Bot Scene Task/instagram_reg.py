...
class InstagramReg():
    ...
    def tf_check_and_rename(file, add=0):
        ...
        if not os.path.isfile(file):
            ...
            add += 1
            InstagramReg.tf_check_and_rename(original_file, add)

    def tf_screen_shots(driver, scroll_delay=0.3):
        path = InstagramReg.tf_check_folder_path("screenshot")
        title = driver.title
        if title != "":
        ...
        if os.path.exists(file_name):
            InstagramReg.tf_check_and_rename(file_name)
    ...
    def tf_source_code(driver):
        path = InstagramReg.tf_check_folder_path("sourcepage")
        title = driver.title
        if title != "":
            title_length = len(str(title))
            if title_length > 26:
        ...
        InstagramReg.tf_File_name = path + "/" + title + ".html"
        if os.path.exists(InstagramReg.tf_File_name):
            InstagramReg.tf_check_and_rename(InstagramReg.tf_File_name)
        pagesource = driver.page_source.encode('ascii', 'ignore')
        soup = BeautifulSoup(pagesource, 'html.parser')
        # Create text file, then write page source to the file
        fh = open(InstagramReg.tf_File_name, 'w')
...
def instagram_reg():
    ALL_PROXIES = InstagramReg.tf_get_proxy()
    Chromedriver = InstagramReg.tf_proxy_driver(ALL_PROXIES)
    running = True
    while running:
        try:
            Chromedriver.get(InstagramReg.tf_To_url())
            if Chromedriver != None:
                InstagramReg.tf_Type_Email_or_Phone(Chromedriver)
                InstagramReg.tf_Type_Full_name(Chromedriver)
                InstagramReg.tf_Type_User_name(Chromedriver)
                InstagramReg.tf_Type_Password(Chromedriver)
                InstagramReg.tf_screen_shots(Chromedriver)
                InstagramReg.tf_source_code(Chromedriver)
                InstagramReg.tf_Type_do_signup(Chromedriver)
                InstagramReg.tf_Type_notification(Chromedriver)
                running = False
