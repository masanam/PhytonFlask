...
class TelegramReg():
    ...
    def tf_check_and_rename(file, add=0):
        ...
        if not os.path.isfile(file):
            ...
            add += 1
            TelegramReg.tf_check_and_rename(original_file, add)

    def tf_screen_shots(driver, scroll_delay=0.3):
        path = TelegramReg.tf_check_folder_path("screenshot")
        title = driver.title
        if title != "":
        ...
        if os.path.exists(file_name):
            TelegramReg.tf_check_and_rename(file_name)
    ...
    def tf_source_code(driver):
        path = TelegramReg.tf_check_folder_path("sourcepage")
        title = driver.title
        if title != "":
            title_length = len(str(title))
            if title_length > 26:
        ...
        TelegramReg.tf_File_name = path + "/" + title + ".html"
        if os.path.exists(TelegramReg.tf_File_name):
            TelegramReg.tf_check_and_rename(TelegramReg.tf_File_name)
        pagesource = driver.page_source.encode('ascii', 'ignore')
        soup = BeautifulSoup(pagesource, 'html.parser')
        # Create text file, then write page source to the file
        fh = open(TelegramReg.tf_File_name, 'w')
    ...
    def tf_Type_verify_otp(Chromedriver):
        try:
            ...
            if opt_value == "":
                time.sleep(Wait_3)
                return TelegramReg.tf_Type_verify_otp(Chromedriver)
        except:
            pass
...
def telegram_reg():
    ALL_PROXIES = TelegramReg.tf_get_proxy()
    Chromedriver = TelegramReg.tf_proxy_driver(ALL_PROXIES)
    running = True
    while running:
        try:
            Chromedriver.get(TelegramReg.tf_To_url())
            time.sleep(Wait_3)
            if Chromedriver != None:
                TelegramReg.tf_Type_phone_number(Chromedriver)
                TelegramReg.tf_do_next_1(Chromedriver)
                TelegramReg.tf_Type_phone_model(Chromedriver)
                TelegramReg.tf_Type_verify_otp(Chromedriver)
                TelegramReg.tf_do_next_2(Chromedriver)
                # tf_Type_get_contact_list(Chromedriver)
                TelegramReg.tf_Type_select_contact(Chromedriver)
                TelegramReg.tf_Type_search_telegram_contact_groups(Chromedriver)
