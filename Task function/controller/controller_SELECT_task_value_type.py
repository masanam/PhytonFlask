...
class NewController():
    ...
    def select_controller():
        checkId = request.form.get('task_value_type_id')
        checkName = request.form.get('task_value_type_name')
        ...
        if request.form.get('search'):            
            ...
            result = DataList.get_task_value_type(start,limit)
        ...
        if request.form.get('Btn_save'):
            ...
            return render_template('tmpl_NEW_task_function.html', title='New Task function',listdata=result, countdata=count, listdataS=list_scene, listdataSC=list_scene_cat, task_value_type_id=checkId, task_value_type_name=checkName, task_function_id=task_function_id, task_function_name=task_function_name, scene_category_id=scene_category_id,scene_id=scene_id)
        else:
            return render_template('tmpl_SELECT_task_value_type.html', title='List task_value_type',listdata=result, countdata=count, task_function_id=task_function_id, task_function_name=task_function_name, scene_category_id=scene_category_id,scene_id=scene_id)    
