...
from model_CSV_task_function import DataCSV
...
class CSVController():
    def CSV_controller():
        ...
        if request.form.get('Btn_save'):
            return render_template('tmpl_LIST_task_function.html', title='List Task function',listdata=result, countdata=count)   
        else:
            return render_template('tmpl_CSV_task_function.html', title='CSV Task function',csvdata=result,countdata=count,filepath=filename,csvrow=row_count)       