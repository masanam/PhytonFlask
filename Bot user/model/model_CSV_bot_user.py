...
class DataCSV():
    ...
    def retrieve_data():
        mc = mysql_connect.cursor()
        query = "SELECT * FROM bot_user ORDER BY bot_user_id DESC LIMIT 0,20 "
        mc.execute(query )
        result = mc.fetchall()
        return result
    def get_data(start,limit):
        mc = mysql_connect.cursor()
        query = "SELECT * FROM bot_user ORDER BY bot_user_id DESC LIMIT %s,%s "
        mc.execute(query,(start,limit) )
        result = mc.fetchall()
        return result    
    def count_data():
        mc = mysql_connect.cursor()
        query = "SELECT * FROM bot_user"
        mc.execute(query)
        mc.fetchall()
        count = mc.rowcount
        return count	
    def search_data(data):
        mc = mysql_connect.cursor()
        query = "SELECT * FROM bot_user WHERE bot_user_name = %s ORDER BY bot_user_id DESC"
    ...
    def save_data(name,description,filename, filerow):
        mc = mysql_connect.cursor()
        query = "INSERT INTO bot_user_file (name, description, file_name, file_row) VALUES (%s,%s,%s,%s)"
