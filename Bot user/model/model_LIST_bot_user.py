...
class DataList():
    ...
    def search_data(data):
        mc = mysql_connect.cursor()
        query = "SELECT * FROM bot_user WHERE bot_user_type = %s"
        mc.execute(query,(data) )
        result = mc.fetchall()
        return result
    def search_data1(data):
        mc = mysql_connect.cursor()
        query = "SELECT * FROM bot_user WHERE country = %s "
        mc.execute(query,(data) )
        result = mc.fetchall()
        return result
    def search_data2(data):
        mc = mysql_connect.cursor()
        query = "SELECT * FROM bot_user WHERE first_name = %s "
        mc.execute(query,(data) )
        result = mc.fetchall()
        return result
