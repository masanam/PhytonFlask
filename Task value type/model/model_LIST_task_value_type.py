...
class DataList():
    def retrieve_data():
        ...
        query = "SELECT * FROM task_value_type LIMIT 0,20 "
        mc.execute(query )
        result = mc.fetchall()
        return result
    ...
    def search_data(data):
        mc = mysql_connect.cursor()
        query = "SELECT * FROM task_value_type WHERE task_value_type_name = %s "
    ...
    def save_data(name,description):
        mc = mysql_connect.cursor()
        now = datetime.now()
        query = "INSERT INTO task_value_type ( task_value_type_name, created_at ) VALUES (%s,%s)"
        data = (name,now)
        mc.execute(query,(data) )
