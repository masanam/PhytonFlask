...
class DataCSV():
    def import_data(file_path):
        ...
        now = datetime.now()
        for row in csv_data:
            mc.execute('INSERT INTO task_function_category(task_function_category_name ,created_at) VALUES ( %s, %s)' , (row,now)  )
        mysql_connect.commit()
        mc.close()

    def retrieve_data():
        mc = mysql_connect.cursor()
        query = "SELECT * FROM task_function_category ORDER BY task_function_category_id DESC LIMIT 0,20 "
        mc.execute(query )
        result = mc.fetchall()
        return result
    ...
    def search_data(data):
        mc = mysql_connect.cursor()
        query = "SELECT * FROM task_function_category WHERE task_function_category_name = %s ORDER BY task_function_category_id DESC"
    ...
    def save_data(name,description,filename, filerow):
        mc = mysql_connect.cursor()
        query = "INSERT INTO task_function_category_file (name, description, file_name, file_row) VALUES (%s,%s,%s,%s)"
