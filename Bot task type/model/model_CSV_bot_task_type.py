...
class DataCSV():
    def import_data(file_path):
        ...
        for row in csv_data:
            mc.execute('INSERT INTO bot_task_type(bot_task_type_name,created_at) VALUES (%s, %s)', (row, now))
        mysql_connect.commit()
        mc.close()

    def retrieve_data():
        mc = mysql_connect.cursor()
        query = "SELECT * FROM bot_task_type ORDER BY bot_task_type_id DESC LIMIT 0,20 "
        mc.execute(query )
        result = mc.fetchall()
        return result
    ...
    def search_data(data):
        mc = mysql_connect.cursor()
        query = "SELECT * FROM bot_task_type WHERE bot_task_type_name = %s ORDER BY bot_task_type_id DESC"
    ...
    def save_data(name,description,filename, filerow):
        mc = mysql_connect.cursor()
        query = "INSERT INTO bot_task_type_file (name, description, file_name, file_row) VALUES (%s,%s,%s,%s)"
